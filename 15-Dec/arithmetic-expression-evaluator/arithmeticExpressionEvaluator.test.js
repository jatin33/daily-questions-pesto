...
describe('arithmeticExpressionEvaluator', () => {
  ...
  it('throws error when expression contains invalid characters', () => {
    ...
    expect(() => arithmeticExpressionEvaluator('(5 + 5) * 2')).toThrow('Parenthesis not supported in input!');

    // For simplicity's sake, we don't want to support exponentiation
    expect(() => arithmeticExpressionEvaluator('10^4')).toThrow('Exponentiation not supported in input!');

    // For simplicity's sake, we don't want to support floating point numbers
    expect(() => arithmeticExpressionEvaluator('10.123 * 2')).toThrow('Floats not supported in input!');
  });

  it('throws error when expression is not string', () => {
    expect(() => arithmeticExpressionEvaluator(12)).toThrow('Input should only be string!');
    expect(() => arithmeticExpressionEvaluator([])).toThrow('Input should only be string!');
    expect(() => arithmeticExpressionEvaluator({})).toThrow('Input should only be string!');
    expect(() => arithmeticExpressionEvaluator(NaN)).toThrow('Input should only be string!');
    expect(() => arithmeticExpressionEvaluator(null)).toThrow('Input should only be string!');
    expect(() => arithmeticExpressionEvaluator()).toThrow('Input should only be string!');
  });
});
